...
router.post('/signin', (req, res) => {
    ...
    User.findOne({ email: email })
        .populate("followers", "_id username profPic profAudio profCoverPic")
        .populate("following", "_id username profPic profAudio profCoverPic")
        .then(savedUser => {
            if (!savedUser) {
            ...
            bcrypt.compare(password, savedUser.password)
                .then(doMatch => {
                    if (doMatch) {
                        ...
                        const { _id, username, email, followers, profPic, profAudio, profCoverPic, following, hobbies } = savedUser;
                        res.json({ token, user: { _id, username, email, profPic, profAudio, profCoverPic, followers, following, hobbies } });
                    }
                    else {
                        return res.status(422).json({ error: "Invalid email or password" });
                    }
