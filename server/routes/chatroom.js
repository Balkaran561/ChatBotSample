...
const addChatLive = (socket, io) => {
    socket.on("add chat", data => {
        const { message, image, sender, inChatRoom, isHiddenChat } = data;
...
const turnOnHiddenModeLive = (socket, io) => {
    socket.on("turn on hidden mode", data => {

        const { chatroom, currentUser } = data;

        Chatroom.findByIdAndUpdate(chatroom._id, {
            isInHiddenMode: true
        }, {
            new: true
        })
            ...
            .exec((err, chatroom) => {
                if (err) {
                    ...
                    io.emit("turn on hidden mode success", chatroom);
                }
            })
    })
}

const turnOffHiddenModeLive = (socket, io) => {
    socket.on("turn off hidden mode", data => {
        ...
        Chatroom.findByIdAndUpdate(chatroom._id, {
            ...
            .exec((err, chatroom) => {
                if (err) {
                    ...
                    Chat.deleteMany({ isHiddenChat: true })
                        .exec((err2) => {
                            if (err2) {
                                console.log(err2);
                            }
                            else {
                                io.emit("turn off hidden mode success", chatroom);
                            }
                        })
                }
            })
...
module.exports.turnOnHiddenModeLive = turnOnHiddenModeLive;
module.exports.turnOffHiddenModeLive = turnOffHiddenModeLive;
