...
const CommentsModal = ({ item, state, deleteComment }) => {
  ...
  return (
    <div>
      <a className="modal-trigger grey-text" href={`#comments-${item._id}`}>
        View {item.comments.length} comments
      </a>

      <div
        id={`comments-${item._id}`}
          ...
          {item.comments.map((record) => {
            return (
              <div className="row comment-row" key={record._id}>
                    ...
                    <Link
                      to={
                        record.postedBy._id == state._id
                          ? "/profile"
                          : `/profile/${record.postedBy.username}`
                      }
                    >
                ...
                {record.postedBy._id == state._id && (
                  <i
                    className="material-icons col s2 red-text"
