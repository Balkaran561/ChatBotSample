...
const Chatrooms = () => {
    ...
    const addChatLive = (message) => {
        socket && socket.emit("add chat", { 
            message, 
            inChatRoom: openedChatroom.chatroom, 
            sender: state._id,
            isHiddenChat:openedChatroom.chatroom.isInHiddenMode?true:false
         });
    }
    ...
    const turnOnHiddenModeLive = () => {
        //setOpenChatroomLoading(true);

        socket && socket.emit("turn on hidden mode", {
            chatroom: openedChatroom.chatroom,
            currentUser: state
        });
    }

    const turnOffHiddenModeLive = () => {
        //setOpenChatroomLoading(true);

        socket && socket.emit("turn off hidden mode", {
            chatroom: openedChatroom.chatroom,
            currentUser: state
        });
    }
    ...
    useEffect(() => {
        socket && socket.on("turn on hidden mode success", chatroom => {
            if (openedChatroom && openedChatroom.chatroom && openedChatroom.chatroom._id === chatroom._id) {
                console.log(chatroom);
        ...
        return () => socket && socket.removeAllListeners("turn on hidden mode success");
    })

    useEffect(() => {
        socket && socket.on("turn off hidden mode success", chatroom => {
            if (openedChatroom && openedChatroom.chatroom && openedChatroom.chatroom._id === chatroom._id) {
                console.log(chatroom);    
           
                var keptChats=openedChatroom.chats.filter(chat=>!chat.isHiddenChat);
                
                setOpenedChatroom({ ...openedChatroom, chatroom, chats:keptChats });
        ...
        return () => socket && socket.removeAllListeners("turn off hidden mode success");
    })
                                ...
                                <div className="card-title row" style={{ borderBottom: "1px solid gray", paddingBottom: "10px" }}>
                                    {
                                        openedChatroom && openedChatroom.chatroom ?
                                            <>
                                                <div className="chatroom-title col s9">{openedChatroom.chatroom.participants.map(parti => parti.username).join(" ")}</div>
                                                ...
                                                {openedChatroom.chatroom.isInHiddenMode
                                                    ?
                                                    <i className="material-icons right col s1"
                                                        style={{ cursor: "pointer",color:"green"}}
                                                        onClick={() => turnOffHiddenModeLive()}>
                                                        remove_red_eye</i>
                                                    :
                                                    <i className="material-icons right col s1"
                                                        style={{ cursor: "pointer", color:"red" }}
                                                        onClick={() => turnOnHiddenModeLive()}>
                                                        remove_red_eye</i>
                                                }
                                                            ...
                                                            chat.sender ?
                                                                chat.sender._id === state._id ?
                                                                    <div className="mine messages">
                                                                        <div className={`message ${chat.isHiddenChat?"#37474f blue-grey darken-3":"  #1976d2 blue darken-1"}`}>
                                                                            {chat.message
                                                                                ? chat.message
                                                                                : chat.image
                                                                    ...
                                                                    <div className="yours messages">
                                                                        ...
                                                                        <div className={`message ${chat.isHiddenChat?"#b0bec5 blue-grey lighten-3":""}`}>
                                                                            {chat.message
                                                                                ? chat.message
                                                                                : chat.image
                                                                                    ? chat.image.substring(chat.image.lastIndexOf('.') + 1, chat.image.length).match(/(jpg|jpeg|png|gif)$/i)
