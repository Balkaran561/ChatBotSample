...
const UserProfile = () => {
    ...
    const [highlightedStories,setHighlightedStories]=useState(null);
    ...
    useEffect(()=>{
        if(userProfile&& userProfile.user && userProfile.user._id){
            fetch(`/highlightedStories/${userProfile.user._id}`, {
                headers: {
                    "Authorization": "Bearer " + localStorage.getItem("jwt")
                }
            })
                .then(res => res.json())
                .then(result => {
                    setHighlightedStories(result.stories);
                })
        }
    },[userProfile])
    ...
    useEffect(() => {
        //cleans up profile audio on changing to another user profile
        if(profileAudio.current){
            profileAudio.current.pause();
    ...
    useEffect(()=>{
        if(userProfile&&userProfile.user&& userProfile.user.profAudio){
            var audio = new Audio(userProfile.user.profAudio);
            audio.loop = true;
    ...
    return (
        <div style={{ maxWidth: "600px", margin: "0px auto" }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", position: "relative" }}>
                {
                    state&&userProfile && userProfile.user && userProfile.user.profCoverPic
                        ?
                        <img className="materialboxed cover-img"
                            src={userProfile.user.profCoverPic} />
                        :
                ...
                <div className="modal-content">
                    {
                        state && highlightedStories && highlightedStories.length > 0 ?
                            <HighlightStoriesSlider currentHighlightStories={highlightedStories} currentStoryUser={userProfile.user} state={state}/>
                            :
                            <></>
                    }
                </div>
